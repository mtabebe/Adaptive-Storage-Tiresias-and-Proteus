...
siteManagerBin="$baseDirectory/Adapt-HTAP/code/build/exe/site_manager_server/release/site_manager_server"
...
usage() {
    ...
    echo "Runs the site manager server"
    ...
    echo "-s S: uses binary S as the site manager server"
    echo "-v V: use logging verbosity V"
    echo "-h: prints help message"
    echo "phases:"
    echo -e "\t0:runs the site manager server"
    exit 0
	fi
}
...
runSiteManager() {
  ...
  echo "$siteManagerBin --flagfile=$flagFile --logtostderr=1 --v="$verbosity" > $logFile 2>&1"
 # sudo perf record -o ${outDir}/${label}.perf
  $siteManagerBin --flagfile=$flagFile --logtostderr=1 --v="$verbosity" > $logFile 2>&1 &
...
runCommandIfInPhase 0 runSiteManager "runs the site manager server"
