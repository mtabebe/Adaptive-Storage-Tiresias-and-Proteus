...
confFile="$baseDirectory/Adapt-HTAP/deployment/configs/single_site_bench.cfg"
singleSiteBenchBin="$baseDirectory/Adapt-HTAP/code/build/exe/single_site_benchmark/release/single_site_benchmark"
...
usage() {
    ...
    echo "Runs the single site benchmark"
...
runBench() {
  ...
  echo "$singleSiteBenchBin --flagfile=$flagFile --logtostderr=1 --v="$verbosity" > $logFile 2>&1"
  $singleSiteBenchBin --flagfile=$flagFile --logtostderr=1 --v="$verbosity" > $logFile 2>&1
  ...
  resultsFile="${outDir}/${label}_results_${now}.res"
  cat $logFile | grep benchmark_statistics.cpp | cut -d "]" -f 2 > $resultsFile
  ln -sf $resultsFile "${outDir}/${label}_results.res"
}

usage

runCommandIfInPhase 0 runBench "runs the benchmark"
