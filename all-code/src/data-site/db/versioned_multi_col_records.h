...
class versioned_multi_col_records {
   public:
    versioned_multi_col_records( const partition_column_identifier& pcid,
    ...
    std::vector<std::shared_ptr<packed_column_records>> get_snapshot_columns(
        uint64_t version ) const;
    std::vector<std::shared_ptr<packed_column_records>> get_current_columns()
        const;
    ...
    std::shared_ptr<packed_column_records> get_snapshot_columns(
        uint64_t version, uint32_t col ) const;
    std::shared_ptr<packed_column_records> get_current_columns(
        uint32_t col ) const;
    std::tuple<std::shared_ptr<packed_column_records>, uint64_t>
        get_current_columns_and_version( uint32_t col ) const;
    ...
    cell_data_type get_col_type( uint32_t col ) const;
   ...
   private:
    ...
    void insert_into_snapshot_map(
        const
        std::vector<std::shared_ptr<packed_column_records>>& record,
        uint64_t                                            version );
    ...
    folly::ConcurrentHashMap<
        uint64_t, std::vector<std::shared_ptr<packed_column_records>>>
                                                       snapshot_records_;
    std::vector<std::shared_ptr<packed_column_records>> current_records_;
    std::vector<bool>                                  dirtied_records_;
