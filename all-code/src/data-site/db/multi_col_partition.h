...
class multi_col_partition : public internal_partition {
   public:
    multi_col_partition();
    multi_col_partition( bool is_sorted );
    ...
    void set_records( std::shared_ptr<versioned_multi_col_records> records );
    ...
    void change_type_from_sorted_column( const multi_col_partition* sorted_col,
                                         uint64_t             version );

    void merge_multi_col_records( multi_col_partition* low_p,
                                  multi_col_partition* high_p,
                                  uint32_t             col_merge_point,
                                  uint64_t             row_merge_point,
                                  uint64_t             merge_version );

    void split_multi_col_records( multi_col_partition* low_p,
                                  multi_col_partition* high_p,
...
class sorted_multi_col_partition : public multi_col_partition {
   public:
    sorted_multi_col_partition();

    partition_type::type get_partition_type() const override;
};


#include "multi_col_partition-inl.h"
