...
class mlp_predictor2_test : public ::testing::Test {};

#if 0
TEST_F( mlp_predictor2_test, dlib_sample_classification_test ) {
    mlp_predictor2 p( false /* is_static */, {1000, 1000} /* max_input */,
...
void check_mlp_model(
    ...
    int num_samples, const std::vector<double>& max_input, double max_pred,
    bool is_static, const std::vector<long>& layer_1_nodes_configs,
    const std::vector<long>&   layer_2_nodes_configs,
    const std::vector<double>& learning_rate_configs,
    const std::vector<double>& momentum_configs, double acceptable_rmse ) {
        ...
        for( long layer_2_nodes : layer_2_nodes_configs ) {
            for( double learning_rate : learning_rate_configs ) {
                for( double momentum : momentum_configs ) {
                    mlp_predictor2 p( is_static, max_input, layer_1_nodes,
                                      layer_2_nodes, learning_rate, momentum,
                                      -max_pred, max_pred );

                    double rmse = test_dlib2_model(
                        p, x_min, x_max, func, noise, num_iters, num_samples );
...
TEST_F( mlp_predictor2_test, single_sigmoid_test ) {
    check_mlp_model(
...
TEST_F( mlp_predictor2_test, double_mean_squared_test ) {
    check_mlp_model(
...
TEST_F( mlp_predictor2_test, triple_mean_abs_sinc_test ) {
    check_mlp_model(
...
TEST_F( mlp_predictor2_test, quadruple_weight_test ) {
    check_mlp_model(
